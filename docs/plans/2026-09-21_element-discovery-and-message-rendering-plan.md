...
- [x] Make answer/code copy controls icon-only and right-aligned, as requested. Preserve action names, tooltips, keyboard focus, exact copy payloads, and pending/success/failure feedback through streaming; verify unit tests and native browser layout at 320/360 px, 12/24 px, and both themes. Six updated unit cases failed before implementation; all 27 focused rendering/Markdown tests now pass. The production-extension E2E case verifies Enter/Space activation, tooltip feedback, clipped live-status text, 34 px controls with 16 px icons, exact right-edge alignment, and no horizontal page overflow across all eight layout variants. Full CI passed (259 unit/integration tests and 28 E2E tests), and the production audit reports zero vulnerabilities. A Chrome accessibility-tree probe exposes the nested status as an unignored polite/atomic live region. Native Side Panel and screen-reader manual acceptance remain unrecorded; this does not complete the plan.
...
- R6, [successful navigating mutations misreported stale](https://github.com/narumiruna/pi-chrome/pull/40#discussion_r4059054663), confirmed P2/in scope and fixed: two integration regressions failed before the fix after actual click/input handlers invalidated the snapshot. Successful injected mutations now retain their results; discovery, inspection, and deferred cross-origin navigation still require current context/snapshot. Five added worker cases cover successful click/type/approved submit, no replay, pre-mutation rejection, replaced discovery, and deferred navigation; native Chrome click/type hash-navigation tests pass.
- R7, [resolved submit overrides missing from fingerprints](https://github.com/narumiruna/pi-chrome/pull/40#discussion_r4059054665), confirmed P2/in scope and fixed: four integration regressions and a native Chrome form/base-URL test failed before the fix. Fingerprints and confirmation now share effective submit-control resolution, recording native resolved `formAction`, override attributes, and associated form identity for buttons, submit inputs, labels, and nested targets. Native Chrome proves base changes with unchanged raw attributes/form action are rejected with zero submissions. Initial focused checks passed (61 unit/integration cases and two browser tests), then all nine new integration cases and two E2E cases passed full CI; reopened tasks/checks are checked again.
- R8, [clipped discovery labels](https://github.com/narumiruna/pi-chrome/pull/40#discussion_r4059081766), confirmed P2/in scope and fixed: three unit regressions and a native Chrome layout test failed before the fix, exposing fully clipped ARIA/associated/nested label text in discovered names. Name collection now requires target/descendant hits for text parents; fallback names and newly visible labels remain usable. Focused checks passed (65 unit/integration tests), then full CI passed with three new unit cases and one native-layout E2E case; reopened tasks are checked again.
- R9, [submitter payload revalidation](https://github.com/narumiruna/pi-chrome/pull/40#discussion_r4059122682), confirmed P2/in scope and fixed: six integration regressions and a native Chrome test failed before the fix when submitter name/value, encoding, or validation settings changed during confirmation. Isolated fingerprints now compare effective submitter name/value, `formenctype`/`formnovalidate`, and associated form encoding/validation settings. These are private comparison metadata, not discovery/model/persistence output; editable field values are not read. Tests cover all six mutations, zero submissions, and absent payload values in discovery output. Focused checks passed (71 unit/integration tests), then full CI passed; reopened tasks are checked again.
- R10, [painted text-range comment 4059173491](https://github.com/narumiruna/pi-chrome/pull/40#discussion_r4059173491), confirmed P2/in scope and fixed: a native Chrome regression failed before the fix, exposing unseen ARIA/associated/nested label text and clipped suffixes from visible containers. Name collection now checks code-point Range geometry and text-parent hits, retaining whitespace between wrapped words and inspecting at most 100 text nodes/256 UTF-16 units per lookup. Five added unit cases cover fallbacks, changed-name reference rejection, descendant occlusion, empty ranges, Unicode offsets, and bounded work. The native browser case additionally verifies actual overflow/indentation, wrapped text, restored visibility, and zero clicks after clipping changes a reference name. Focused 76 unit/integration cases and full CI (264 unit/integration plus 29 E2E tests) pass; reopened automated tasks/checks are checked again. Reviewed the entire follow-up diff: injected helpers remain self-contained, native selector compatibility is retained, and no permissions, API, storage, dependencies, confirmation, or replay behavior changed.
- R11, [effective disabled-state comment 4059495898](https://github.com/narumiruna/pi-chrome/pull/40#discussion_r4059495898), confirmed P2/in scope and fixed: four unit regressions and a native Chrome test failed before the fix because enabling a disabled fieldset changed available actions without changing the control's own disabled attribute. Fingerprints now include native `:disabled` state. Tests cover inspection/click/type, changes during the final worker assertion, no mutation before rediscovery, successful actions after rediscovery, enabling/disabling, and the first-legend exception. Focused 80 unit/integration cases and full CI (268 unit/integration plus 30 E2E tests) pass. The complete five-file diff preserves selector behavior and adds no permissions, sensitive metadata, dependencies, persistence, or replay; reopened automated checks are checked again.
- Rechecked all feedback pages at `deaeb27`: prior review items remain settled; R11 is the only new substantive request. No duplicate replies are needed for unchanged outcomes.
...
- Dependencies: exact `marked@18.0.13` and `dompurify@3.4.15`; extension-CSP probe passed headings, tables, partial fences, raw HTML, unsafe links, and remote-image rejection. Current whole-feature artifact is 3,150,690 bytes (+90,550 bytes, approximately 3.0%, versus baseline; the effective-disabled-state fix adds 23 bytes to the previously verified 3,150,667-byte artifact). `npm audit --omit=dev` reports zero vulnerabilities; the earlier full `npm audit` also reported zero vulnerabilities, and dependencies are unchanged.
- Rendering extraction passed the three pre-existing projection/image tests before behavior changes. Final `npm run ci` passed on Node.js 26.8.2: lint; 268 unit/integration tests across 26 files; browser bundle probe; production build/typecheck/artifact audit; 30 Playwright tests. `git diff --check` passed.
...
- Manual evidence updated on 2026-09-21: the user reports “實測看起來都正常” (actual testing looks normal) after the icon-only/right-aligned copy update. Chrome version, account tier, and individual results for `docs/manual-acceptance.md` cases 14–15 remain unrecorded. Preserve this successful report without inventing missing details. Keep the manual record/completion checks open and retain this plan until the acceptance record is complete.
