...
| Lifecycle leaks | Session shutdown closes sockets and pending work; reconnect backoff and heartbeat timers are cancelled; start/stop is idempotent. Popup revocation waits for pi to persist the change and acknowledge it by closing the connection. | Abrupt process termination relies on the operating system to release the loopback port. |
