...
4. Check **Current page** above the prompt and send your request. The agent can answer without a page, list web tabs and suggest switching to one, or open an HTTP(S) URL when your task needs it. Switching tabs or opening a website requires your confirmation (or a matching prior approval in Balanced/Convenient) and site access; a search query is sent to the search provider only after its exact destination URL is approved. On an ordinary web page, Send still preflights current-site access before the model decides whether to use page tools.
...
9. When Pi requests a bookmark read, review and confirm the search or recent-item limit when the dialog appears. Balanced/Convenient can reuse an identical approval; Chrome asks for the optional bookmark permission the first time.

**Browser action confirmations:** Settings offers **Strict** (confirm every protected action), **Balanced** (default on new installations; remember an approved matching action until Chrome restarts), and **Convenient** (remember it across restarts until cleared). Existing installations start in Strict until a mode is selected. Approvals match the action and target: changed WebMCP arguments, bookmark queries, or destination URLs ask again. Submits, downloads, and clicks without a stable target always ask; the same WebMCP call may still have side effects. Use **Clear remembered approvals** in Settings or switch modes to revoke shortcuts. Chrome's own permission prompts and runtime access checks remain separate.
...
The agent can read visible text and selection, discover visible interactive elements, capture the visible viewport, click or type using discovered element references or CSS selectors, navigate, and use page WebMCP tools when available. `browser_read_page` accepts optional `selector` (first rendered HTML match, such as `main` or `article`) and `offset` (UTF-16 text index, starting at 0). Without a selector it reads the page body. A long read returns `nextOffset` when truncated; pass it back with the same selector to continue. Invalid, missing, or non-rendered selectors fail rather than falling back to the full page. This reads rendered DOM text, including text outside the viewport, not raw HTML, form field values, or iframe/Shadow DOM contents. To use WebMCP, enable `chrome://flags/#enable-webmcp-testing`, relaunch Chrome, and reload the page; the page must provide WebMCP tools. See [WebMCP status](docs/webmcp.md) for details.
...
- Form submissions and downloads always prompt; cross-origin links/navigation and WebMCP calls require either fresh confirmation or a matching approval in the selected confirmation mode. No cached approval replays a failed mutation.
- A request created before navigation or a visible-tab change is rejected as stale. Chat and approved bookmark reads do not require a supported page; a no-page turn cannot use page tools even after a tab switch.
