...
The agent can read visible text and selection, discover visible interactive elements, capture the visible viewport, click or type using discovered element references or CSS selectors, navigate, and use page WebMCP tools when available. To use WebMCP, enable `chrome://flags/#enable-webmcp-testing`, relaunch Chrome, and reload the page; the page must provide WebMCP tools. See [WebMCP status](docs/webmcp.md) for details.
