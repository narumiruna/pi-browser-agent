...
Pi Browser Agent does not continuously collect page content or maintain a background browsing-history service. It does not read Chrome internal pages, Web Store pages, local files, or PDFs; this release provides no file/PDF import. When the agent requests web tabs, it receives up to 20 current-window HTTP(S) tab titles and origins without reading those pages. Switching to one or opening an HTTP(S) tab requires user confirmation and site access; protected and local tabs are excluded. If the user enters a search query, the query is included in the confirmed destination URL and shared with the search provider when the tab opens. Page access occurs to provide the user-facing assistant features and is subject to Chrome permissions and Pi Browser Agent's runtime checks. Unsent selected-element chips remain only in memory and are cleared on relevant tab or session changes. They are shared only after the user selects **Send**.
