# Privacy Policy for Pi Chrome

**Effective date:** September 22, 2026

Pi Chrome is a Chrome extension that provides an AI assistant for the page the user is viewing. This policy explains what information Pi Chrome handles, why it is handled, where it is stored, and when it is shared.

## Summary

- Pi Chrome has no developer-operated backend and includes no advertising or analytics service.
...
- Pi Chrome does not sell user data or use it for advertising, profiling, creditworthiness, or lending.
...
## Information Pi Chrome handles

### Authentication information

Pi Chrome stores credentials supplied for user-selected AI providers. Depending on the provider, this can include an API key or OAuth access and refresh tokens. OpenAI Codex account login also stores the ChatGPT account identifier contained in the credential. Credentials are stored in `chrome.storage.local`, restricted to trusted extension contexts, and are not included in conversation transcripts or exposed to websites.
...
When required by a user request, Pi Chrome may handle the active tab's URL and approved page context, including visible text, selected text, descriptions of visible interactive elements, screenshot content, and results returned by page or WebMCP tools. It targets only the active, visible HTTP or HTTPS tab in the focused Chrome window.

Pi Chrome does not continuously collect page content or maintain a background browsing-history service. Page access occurs to provide the user-facing assistant features and is subject to Chrome permissions and Pi Chrome's runtime checks.

### Bookmark information

If the user explicitly confirms a bookmark operation and grants the optional permission, Pi Chrome may search bookmark titles and URLs or read a bounded list of recent bookmarks. Every bookmark read requires confirmation. Pi Chrome does not create, modify, move, or delete bookmarks.

### Settings and permission records

Pi Chrome stores the selected provider and model, interface preferences, user-edited instructions, the active session identifier, and exact origins approved through user actions. A context-menu selection that has not yet reached the Side Panel can be held temporarily in `chrome.storage.session`.

### Voice input

Voice input uses Chrome's Web Speech service. Spoken audio may be processed by Chrome's configured speech provider. Pi Chrome receives an editable transcript and sends it to the selected AI provider only when the user chooses **Send**. Pi Chrome does not store the spoken audio.

## How information is used

Pi Chrome uses information only to:
...
Pi Chrome can share information in these circumstances:
...
Pi Chrome does not sell user data, transfer it to data brokers or advertising platforms, or use it for personalized advertising, creditworthiness, or lending.
...
No method of storage or transmission is completely secure, and Pi Chrome cannot guarantee the security practices of external providers or websites.
...
This policy may be updated when Pi Chrome's data practices or legal obligations change. Material changes will be published at this URL with a revised effective date.

## Contact

Questions or privacy requests can be submitted through the public issue tracker:

https://github.com/narumiruna/pi-chrome/issues
